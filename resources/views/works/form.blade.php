@if(isset($user))
    @php
        $option = $option ?? "";
        $workId = isset($work) ? $work->id : null;
    @endphp

    <input type="hidden" name="option" value="{{ $option }}">
    <input type="hidden" name="work_unit" value="{{ $workId }}">
    ...
    @if($option !== "createStart")
        <div class="form-group">
            <label for="work_done" class="control-label">Work Description</label>
            <textarea class="form-control" placeholder="What did you exactly do?" rows="5" name="work_done">{{ $work->work_done }}</textarea>
        </div>
    @else
    ...
    @if(Auth::user()->is_manager)
        <div class="form-group label-static is-empty">
            ...
            <input type="number" class="form-control" name="rate" id="rate" placeholder="10.00" step="0.01" value="{{ number_format($work->rate ?: $user->rate, 2) }}">
        </div>
    @endif
    ...
    @if($option !== "createStart" && $option !== "createEnd")
        <div class="form-group label-static is-empty">
            ...
            <input type="datetime-local" class="form-control" id="start" name="start" value="{{ ($work->start ?: (new \Carbon\Carbon())->subHour())->format('Y-m-d\TH:i') }}">
        </div>

        <div class="form-group label-static is-empty">
            <label for="end" class="control-label">End</label>
            <input type="datetime-local" class="form-control" id="end" name="end" value="{{ ($work->start !== null ? $work->start->addMinutes($work->hours * 60) : new \Carbon\Carbon())->format('Y-m-d\TH:00') }}">
        </div>
    @endif
